...
export function normaliseFingerprint(obj) {
  ...
  Object.keys(obj).forEach((key) => {
    ...
    if (match) {
      ...
      console.log(
        `Ignoring file ${key} as it does not match known asset file pattern`
      );
    }
  });
...
export function diffTotals(prTotals, masterTotals) {
  console.log({ prTotals });
  ...
  Object.keys(prTotals).forEach((fileType) => {
    console.log(fileType);

    diff[fileType].raw = prTotals[fileType].raw - masterTotals[fileType].raw;
    diff[fileType].gzip = prTotals[fileType].gzip - masterTotals[fileType].gzip;
  });
  return diff;
}
...
export async function installDependencies() {
  ...
  if (fs.existsSync("package-lock.json")) {
    ...
    if (packageLock.lockfileVersion === 2 && !npmVersion.startsWith("7")) {
...
`;
  data.forEach((item) => {
    table += `${item.file}|${prettyBytes(item.raw, {
      signed: true,
    })}|${prettyBytes(item.gzip, { signed: true })}\n`;
  });

  return table;
}

export function buildOutputText(output, totalSizeDiff) {
  ...
  if (totalSizeDiff) {
    const totalsRows = [
      { file: "js", ...totalSizeDiff.js },
      { file: "css", ...totalSizeDiff.css },
    ];
