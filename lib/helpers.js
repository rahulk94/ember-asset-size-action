...
export function buildOutputText(output, prTotals, masterTotals) {
  ...
  if (prTotals) {
    const totalsRows = [
      { file: 'js', ...prTotals.js },
      { file: 'css', ...prTotals.css },
    ];

    outputText += `PR total Sizes 📊:\n\n${reportTable(totalsRows)}`;
  }

  if (masterTotals) {
    const totalsRows = [
      { file: 'js', ...masterTotals.js },
      { file: 'css', ...masterTotals.css },
    ];

    outputText += `Master total sizes 📊:\n\n${reportTable(totalsRows)}`;
  }
